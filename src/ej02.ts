...
export function parseAndAdd(a: string, b: string): number {
  const valor1 = Number(a);
  const valor2 = Number(b);
  if (Number.isNaN(valor1) || Number.isNaN(valor2)){ throw new Error("invalid number");}
...
export function onlyNumbers(items: Array<string | number | boolean>): number[] {
  return items.filter(item => typeof item === "number") as number[];
}
...
export function groupByType(items: Array<string | number | boolean>): TypeGroups {
  ...
  for(const item of items){
    ...
    }else {groups.booleans.push(item)}
  };
  return groups;
}
